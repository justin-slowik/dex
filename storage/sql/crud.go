...
func (c *conn) CreateAuthRequest(a storage.AuthRequest) error {
	_, err := c.Exec(`
		insert into auth_request (
			...
			claims_groups, claims_locale, claims_zoneinfo,
	...
	`,
		...
		encoder(a.Claims.Groups), a.Claims.Locale, a.Claims.ZoneInfo,
	...
	return c.ExecTx(func(tx *trans) error {
		...
		_, err = tx.Exec(`
			...
			set
				...
				claims_groups = $13, claims_locale = $14, claims_zoneinfo = $15,
...
func getAuthRequest(q querier, id string) (a storage.AuthRequest, err error) {
	...
	`, id).Scan(
		...
		&a.ForceApprovalPrompt, &a.LoggedIn,
		&a.Claims.UserID, &a.Claims.Username, &a.Claims.Email, &a.Claims.EmailVerified,
		decoder(&a.Claims.Groups), &a.Claims.Locale, &a.Claims.ZoneInfo,
...
func (c *conn) CreateAuthCode(a storage.AuthCode) error {
	_, err := c.Exec(`
		insert into auth_code (
			...
			claims_zoneinfo, connector_id, connector_data,
			expiry
		)
	...
	`,
		...
		a.Claims.Locale, a.Claims.ZoneInfo, a.ConnectorID, a.ConnectorData, a.Expiry,
	)
...
func (c *conn) GetAuthCode(id string) (a storage.AuthCode, err error) {
	err = c.QueryRow(`
		select
			...
			claims_locale, claims_zoneinfo, connector_id, connector_data,
			expiry
...
func (c *conn) CreateRefresh(r storage.RefreshToken) error {
	...
	`,
		...
		encoder(r.Claims.Groups), r.Claims.Locale, r.Claims.ZoneInfo,
...
func scanRefresh(s scanner) (r storage.RefreshToken, err error) {
	err = s.Scan(
		...
		&r.Claims.UserID, &r.Claims.Username, &r.Claims.Email, &r.Claims.EmailVerified,
		decoder(&r.Claims.Groups), &r.Claims.Locale, &r.Claims.ZoneInfo,
		&r.ConnectorID, &r.ConnectorData,
		&r.Token, &r.CreatedAt, &r.LastUsed,
	)
	if err != nil {
